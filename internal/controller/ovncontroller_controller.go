...
func (r *OVNControllerReconciler) reconcileNormal(ctx context.Context, instance *ovnv1.OVNController, helper *helper.Helper) (ctrl.Result, error) {
	...
	//
	OVNinputHash, OVNhashChanged, OVNerr := r.createHashOfInputHashes(ctx, instance, OVNConfigMapVars, ovnv1.OVNInputHash)
	OVSinputHash, OVShashChanged, OVSerr := r.createHashOfInputHashes(ctx, instance, configMapVars, ovnv1.OVSInputHash)
	...
	Log.Info(fmt.Sprintf("ARNAU - NetworkAttachments: %s", instance.Spec.NetworkAttachment))
	Log.Info(fmt.Sprintf("ARNAU - nicMappings: %s", instance.Spec.NicMappings))
	Log.Info(fmt.Sprintf("ARNAU - bondConfig: %+v", instance.Spec.BondConfiguration))
...
func (r *OVNControllerReconciler) generateServiceConfigMaps(
	...
	extraScriptsTemplateParameters["OVNRemoteProbeInterval"] = instance.Spec.ExternalIDS.OvnRemoteProbeInterval
